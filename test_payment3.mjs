...
async function testPaymentInit() {
  try {
    ...
    const transactionResponse = await axios.get('http://localhost:5000/api/transactions/93591403e534b6e062e93eee96db6692', {
      headers: { 'Cookie': sessionCookie },
      withCredentials: true
    });

    console.log('Transaction status:', transactionResponse.data.transaction.status);
